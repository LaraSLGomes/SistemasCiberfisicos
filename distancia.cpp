...
// Funções para ler temperatura e umidade do sensor DHT (nomes corrigidos)
...
// Retransmitir a mensagem para o receiverMac (se aplicável ao seu projeto)
...
memcpy(peerInfo.peer_addr, receiverMac, 6); // Usando receiverMac para adicionar o peer
peerInfo.channel = 0;
...
myNodeInfo.temperature = getTemperature(); // Chamada corrigida
myNodeInfo.humidity = getHumidity();     // Chamada corrigida
...
dht.begin();
Serial.println("Inicialização completa! Lendo valores de temperatura e umidade do sensor."); // Mensagem corrigida
}
...
Serial.print("Lendo Sensor - Temperatura: "); // Mensagem corrigida
...
// Enviar mensagem
digitalWrite(LED_PIN, HIGH);
// Agora, o esp_now_send está direcionado para 'receiverMac' em vez de broadcastAddress
